...
def index(request):
    logger.error('ENVIRONMENT: %s' % settings.ENVIRONMENT)
    logger.error('REDIRECT_HOST: %s' % settings.REDIRECT_HOST)
    logger.error('REDIRECT_URI: %s' % settings.REDIRECT_URI)
    logger.error('REDIRECT_BASE_URL: https://%s' % settings.REDIRECT_HOST)
...
def oauth(request):
    ...
    logger.error('state token: %s' % auth_client.state_token)
...
def callback(request):
    ...
    logger.error('auth code: %s' % auth_code)
    logger.error('realm_id: %s' % realm_id)
    ...
    try:
        ...
        logger.error('access token: %s' % auth_client.access_token)
        logger.error('refresh token: %s' % auth_client.refresh_token)
        ...
        print(e)
    settings.QB_CONFIG.write_access_config(auth_client)
